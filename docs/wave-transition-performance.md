...
`npm test` covers all 20 regression suites. The visual-pipeline suite verifies that a moving/fading label is painted only once, blitted every live frame and no longer rendered after expiry. Music score and lifecycle regressions remain unchanged and pass.
