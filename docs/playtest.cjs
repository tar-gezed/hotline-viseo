...
(async()=>{
 ...
 await page.screenshot({path:path.join(__dirname,'final-menu.png')});
 await page.keyboard.press('Enter');
 await page.waitForTimeout(500);
 await page.screenshot({path:path.join(__dirname,'final-ready.png')});
 ...
 for(let i=0;i<160;i++){
  ...
  if(info.kills>bestKills){bestKills=info.kills;await page.screenshot({path:path.join(__dirname,'final-gameplay.png')});}
  if(i===45&&bestKills<0)await page.screenshot({path:path.join(__dirname,'final-gameplay.png')});
 ...
 await page.screenshot({path:path.join(__dirname,'final-end.png')});
 if(!fs.existsSync(path.join(__dirname,'final-gameplay.png')))await page.screenshot({path:path.join(__dirname,'final-gameplay.png')});
 ...
 await page.screenshot({path:path.join(__dirname,'compare-plan.png'),fullPage:true});
 await page.locator('[data-mode="materials"]').click();await page.waitForTimeout(200);await page.screenshot({path:path.join(__dirname,'compare-materials.png'),fullPage:true});
 await page.locator('[data-mode="interiors"]').click();await page.waitForTimeout(200);await page.screenshot({path:path.join(__dirname,'compare-interiors.png'),fullPage:true});
 await page.locator('[data-mode="gameplay"]').click();await page.waitForTimeout(500);await page.screenshot({path:path.join(__dirname,'compare-gameplay.png'),fullPage:true});
 result.errors=errors;fs.writeFileSync(path.join(__dirname,'playtest-result.json'),JSON.stringify(result,null,2));console.log(result);
 await browser.close();
