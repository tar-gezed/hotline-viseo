...
(async () => {
  ...
  try {
    ...
    for (const [width, height] of [[1280, 720], [1440, 900], [1920, 1080], [3440, 1440]]) {
      ...
      await click(4); await expectState('MENU_CREDITS'); await shot('credits-' + size);
      ...
      await key('Escape'); await key('ArrowDown'); await key('Enter'); await expectState('MENU_CONTROLS');
      ...
      await key('Escape'); await expectState('MENU_TITLE');
      await click(2); await expectState('MENU_AUDIO');
      await key('ArrowLeft');
      ...
      await key('Escape'); await click(3); await expectState('MENU_TOOLS'); await shot('tools-' + size);
    ...
    await pad(13); await pad(0); await expectState('MENU_CONTROLS');
    ...
    await pad(1); await expectState('MENU_TITLE');
    await pad(12); await pad(12); await pad(0, 400); await expectState('MENU_MASK');
    ...
    await click(3); await expectState('MENU_TOOLS'); await click(0);
    await page.waitForURL('**/maps.html');
    await page.goto(url); await expectState('MENU_TITLE');
    await page.waitForTimeout(400); await click(3); await click(1);
