...
- **Dependencies:** **No runtime package installation or CDN imports.** The game runs as a static GitHub Pages application. Solo has no external dependencies; multiplayer explicitly opts into a locally vendored Trystero MQTT bundle (see docs/multiplayer-architecture.md). The optional Windows friends-testing launcher uses a local `cloudflared` executable for Quick Tunnels; that tool is not bundled or committed.
...
The regression runner currently includes 31 suites. Browser acceptance checks
